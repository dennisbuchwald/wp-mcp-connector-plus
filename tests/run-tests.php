...
foreach ( $markup_samples as $label => $markup ) {
	...
	$tree   = dbw_connector_blocks_to_tree( $blocks, array(), true );

	$errors  = array();
	$rebuilt = dbw_connector_tree_to_blocks( $tree, '', $errors );
...
$tree   = dbw_connector_blocks_to_tree( $blocks, array(), false );
...
$tree_full = dbw_connector_blocks_to_tree( $blocks, array(), true );
...
$node = dbw_connector_blocks_at_path( $blocks, array( 0, 0, 1 ) );
t_ok( $node && 'B' === ( $node['attrs']['heading'] ?? '' ), 'path 0.0.1 resolves to the second card' );

t_ok( null === dbw_connector_blocks_at_path( $blocks, array( 0, 0, 9 ) ), 'out-of-range path returns null' );
t_same( array( 2, 0, 1 ), dbw_connector_path_parse( '2.0.1' ), 'path string parses to segments' );
t_same( null, dbw_connector_path_parse( '2..1' ), 'malformed path is rejected' );
t_same( null, dbw_connector_path_parse( 'a.b' ), 'non-numeric path is rejected' );

$tree = dbw_connector_blocks_to_tree( $blocks );
...
// set_attrs
$result = dbw_connector_apply_ops(
	$base,
	array(
		array(
			'op'    => 'set_attrs',
...
// remove
$result = dbw_connector_apply_ops( $base, array( array( 'op' => 'remove', 'path' => '0' ) ) );
...
// move
$result = dbw_connector_apply_ops( $base, array( array( 'op' => 'move', 'path' => '0', 'to' => '1' ) ) );
t_ok( ! is_wp_error( $result ), 'move succeeds' );
t_same( 'dbw-base/hero', $result['blocks'][1]['blockName'], 'move relocates the block' );

// error cases
$result = dbw_connector_apply_ops( $base, array( array( 'op' => 'nope', 'path' => '0' ) ) );
t_ok( is_wp_error( $result ), 'unknown op is rejected' );

$result = dbw_connector_apply_ops( $base, array( array( 'op' => 'remove', 'path' => '99' ) ) );
t_ok( is_wp_error( $result ), 'out-of-range path is rejected' );

$result = dbw_connector_apply_ops( $base, array( array( 'op' => 'remove', 'path' => 'bad' ) ) );
...
$v       = dbw_connector_validate_blocks( $wrapped );
...
$errors = array();
dbw_connector_tree_to_blocks( $tree, '', $errors );
...
$errors                  = array();
$blocks                  = dbw_connector_tree_to_blocks( $tree, '', $errors );
...
$outline = dbw_connector_blocks_to_outline( parse_blocks( $markup ) );
...
$outline = dbw_connector_blocks_to_outline( parse_blocks( '<!-- wp:core/paragraph --><p>Ein längerer Fließtext für das Label</p><!-- /wp:core/paragraph -->' ) );
...
t_ok( ! isset( $by_name['core/paragraph'] ), 'scope "dbw" excludes core blocks' );
...
$all = dbw_connector_build_catalog( 'all' );
t_ok( count( $all ) > count( $catalog ), 'scope "all" includes core blocks' );

// ---------------------------------------------------------------------
t_group( 'Block detail' );

$described = dbw_connector_describe_blocks( array( 'dbw-base/hero', 'dbw-base/gibtsnicht' ) );
...
t_same( 4, dbw_connector_count_blocks( $blocks ), 'nested blocks are counted' );
t_same(
	array( 'dbw-base/section', 'dbw-base/cards', 'dbw-base/card-item', 'dbw-base/card-item' ),
	dbw_connector_block_names( $blocks ),
...
t_same( 2, dbw_connector_count_blocks( $blocks ), 'whitespace null-blocks are not counted' );
...
$result = dbw_connector_apply_ops( $blocks, array( array( 'op' => 'remove', 'path' => '1' ) ) );
$names  = dbw_connector_block_names( $result['blocks'] );
